...
export function getCachedOrdersAgg(startDate, endDate) {
  return unstable_cache(
    ...
    ['orders-agg-v5', startDate, endDate], // v5: internal-api + productManager
    { tags: ['dashboard', 'orders', `orders-${startDate}-${endDate}`], revalidate: DAY }
  )();
}

// 마켓 집계 (기간별 캐시) — MARKET_STARTED_AT 기준 (해당 기간에 시작한 마켓)
export function getCachedMarketsAgg(startDate, endDate, durationType = 'MARKET_STARTED_AT') {
  return unstable_cache(
    async () => {
    ...
    ['markets-agg-v3', startDate, endDate, durationType], // v3: internal-api
