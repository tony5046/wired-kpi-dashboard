...
import { fetchAllOrders, aggregateOrders } from './wired-admin';
...
// 주문 집계 결과 (기간별 캐시). v2: JSON 페이징 API 사용 (10k 한도 없음)
export function getCachedOrdersAgg(startDate, endDate) {
  return unstable_cache(
    async () => {
      const { rows, totalRow } = await fetchAllOrders(startDate, endDate);
      const agg = aggregateOrders(rows);
      return { ...agg, totalRow };
    },
    ['orders-agg-v2', startDate, endDate],
